...
import job.upload.UploadImages;
...
public class Main {
    ...
    public static void main(String[] args) {
        if(args.length != 4 && (args.length == 3 && !"UploadImage".equals(args[0]))) {
            System.out.println("Usages1: java -jar <program> <SearchType> <inputFile(file)> <outputPath(dir)> <taskName>");
            ...
            System.out.println("Usages2: java -jar <program> UploadImage <Path(dir)>");
        }else {
            if("UploadImage".equals(args[0])) {
                try  {
                    UploadImages.uploadImage(args[1]);
                }catch (Exception e) {
                    e.printStackTrace();
                }
            }else {
